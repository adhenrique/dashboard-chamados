    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          className="justify-start border-border text-left font-normal hover:bg-secondary hover:text-accent"
        >
      ...
      <PopoverContent align="end" className="flex w-auto flex-col p-0 md:flex-row">
        <div className="flex flex-col border-b border-border bg-muted/20 p-2 md:border-b-0 md:border-r">
          <p className="px-3 py-2 text-[10px] font-bold uppercase text-muted-foreground">Atalhos</p>
          {presets.map((preset) => (
            <Button
              key={preset.label}
              variant="ghost"
              size="sm"
              className={cn(
                'my-[2px] justify-start font-normal',
