...
import {differenceInDays, differenceInHours, subMonths, format, isSameMonth, isSameDay } from "date-fns";
...
export const transformTicketsToDepartmentData = (
    ...
    return Object.keys(groups).map(deptName => {
        ...
        const avgTime = data.closedCount > 0
            ? Math.round(data.totalDurationHours / data.closedCount)
            : 0;
